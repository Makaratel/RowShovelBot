...
def get_cost(message, user, active):
    ...
    get_lands(message, user, active)

def get_lands(message, user, active):
    user.lands.append(active)
    d.DAO.bd_task(d.DAO.set_lands, message.chat.id, str(user.lands))
