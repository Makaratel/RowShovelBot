...
def to_love(message):
    ...
    if user.marriage == True:
        ...
        world_koef = s.WORLDS_KOEFS[user.world]
        d.DAO.bd_task(d.DAO.set_marriage, message.chat.id, True)
        bot.send_message(message.chat.id, 'Теперь вы женаты. Поздравляем!')

        if user.cash < s.COST_WEDDING * world_koef:
            d.DAO.bd_task(d.DAO.set_cash, message.chat.id, 0)
            d.DAO.bd_task(d.DAO.set_debt, message.chat.id, user.debt + s.COST_WEDDING * world_koef - user.cash)
        else:
            d.DAO.bd_task(d.DAO.set_cash, message.chat.id, user.cash - s.COST_WEDDING * world_koef)
    else:
        world_koef = s.WORLDS_KOEFS[user.world]
        d.DAO.bd_task(d.DAO.set_marriage, message.chat.id, True)
        d.DAO.bd_task(d.DAO.set_total_outcome, message.chat.id, user.total_outcome - s.COST_CHILD * world_koef * user.childs)
        d.DAO.bd_task(d.DAO.set_flow, message.chat.id, user.flow + s.COST_CHILD * world_koef * user.childs)
        bot.send_message(message.chat.id, 'Теперь вы женаты. Поздравляем!')
...
def to_child(message):
    user = p.Person.get_data(message.chat.id)
    world_koef = s.WORLDS_KOEFS[user.world.lower()]
    ...
    if user.gender == 'мужской' and user.marriage == False:
        ...
        d.DAO.bd_task(d.DAO.set_total_outcome, message.chat.id, user.total_outcome + s.COST_CHILD * world_koef)
        d.DAO.bd_task(d.DAO.set_flow, message.chat.id, user.flow - s.COST_CHILD * world_koef)
